        ...
        for cnt, data_dict in enumerate(data_loader):
            ...
            for cat in dsets.category:
                ...
                for pred in result:
                    ...
                    difficult.text = "0"
                    score_ = ET.SubElement(object,"score")
                    score_.text = str(score)
                    bndbox = ET.SubElement(object,"bndbox")
                    xmin = ET.SubElement(bndbox,"xmin")
                    xmin.text = str(np.min(box[:,0]))
                    ymin = ET.SubElement(bndbox,"ymin")
                    ymin.text = str(np.min(box[:,1]))
                    xmax = ET.SubElement(bndbox,"xmax")
                    xmax.text = str(np.max(box[:,0]))
                    ymax = ET.SubElement(bndbox,"ymax")
                    ymax.text = str(np.max(box[:,1]))
            ...
            cv2.imwrite('test/'+img_id+'.png', ori_image)
