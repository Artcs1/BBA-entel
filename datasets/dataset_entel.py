...
class ENTEL(BaseDataset):
    def __init__(self, data_dir, phase, input_h=None, input_w=None, down_ratio=None):
        super(ENTEL, self).__init__(data_dir, phase, input_h, input_w, down_ratio)
        self.category = ['firma','fecha']
    ...
    def load_annotation(self, index):
        ...
        for obj in target.iter('object'):
            ...
            for obj2 in obj.iter('bndbox'):
                box_xmin = int(obj2.find('xmin').text)  # bbox
                box_ymin = int(obj2.find('ymin').text)
                box_xmax = int(obj2.find('xmax').text)
                box_ymax = int(obj2.find('ymax').text)
            mbox_cx =  (box_xmax + box_xmin)/2# rbox
            mbox_cy =  (box_ymax + box_ymin)/2
            mbox_w =   (box_xmax - box_xmin)
            mbox_h =   (box_ymax - box_ymin)
            mbox_ang = 0
            ...
            valid_cat.append(self.cat_ids[obj.find('name').text])
